...
import 'package:i9vate_sales_force/app/infrastructure/http/http.dart';
...
void main() {
  ...
  group('shared', () {
    test(
      'Should throw ServerError if invalid method is provider.',
      () async {
  ...
  group(
    ...
    () {
      ...
      void mockError() {
        mockRequest().thenThrow(Exception('Error generic.'));
      }
      ...
      test(
        'Should call post with corret values',
        () async {
          await sut.request(
      ...
      test(
        'Should return null if post return 200 with no data',
        () async {
      ...
      test(
        'Should return BadRequest if post return 400',
        () async {
          mockResponse(400);
      ...
      test(
        'Should return BadRequest if post return 400 with body empity',
        () async {
      ...
      test(
        'Should return ForbiddenError if post return 403',
        () async {
          mockResponse(403);
      ...
      test(
        'Should return ServerError if throws',
        () async {
          mockError();
